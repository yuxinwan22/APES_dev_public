...
class DownSample(nn.Module):
    ...
    def forward(self, x):
        ...
        attention = self.softmax(energy / scale_factor)
        # attention.shape == (B, H, N, N)
        self.attention = torch.sum(attention, dim=-2)
        # self.attention.shape == (B, H, N)
        self.idx = self.attention.topk(self.k, dim=-1)[1]
        # idx.shape == (B, H, K)
...
class DownSampleWithSigma(nn.Module):
    ...
    def forward(self, x):
        ...
        attention = self.softmax(energy / scale_factor)
        # attention.shape == (B, H, N, 1, K)
        self.attention = torch.std(attention, dim=-1, unbiased=False)[:, :, :, 0]
        # self.attention.shape == (B, H, N)
        self.idx = self.attention.topk(self.k, dim=-1)[1]
        # idx.shape == (B, H, M)
...
class ModelNetModel(nn.Module):
    def __init__(self, neighbor2point_enable, neighbor2point_egdeconv_emb_K, neighbor2point_egdeconv_emb_group_type, neighbor2point_egdeconv_emb_conv1_in,
        ...
        if neighbor2point_enable:
            ...
            num_layers = len(neighbor2point_K)
        ...
        self.linear1 = nn.Sequential(nn.Linear(1024*num_layers, 1024), nn.BatchNorm1d(1024), nn.LeakyReLU(negative_slope=0.2))
