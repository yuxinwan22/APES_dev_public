...
import pkbar
from utils.visualization import visualize_modelnet_downsampled_points
import gdown
...
def test(local_rank, config):
    ...
    with torch.no_grad():
        if rank == 0:
            print(f'Print Results: {config.test.print_results} - Visualize Downsampled Points: {config.test.visualize_downsampled_points.enable}')
